...
The `mtcars` dataset is comes with R.  For information on it simply type `help(mtcars)`.  The variables of interest here are `am` (transmission; 0 = automatic, 1 = manual), `mpg` (miles per gallon), `qsec` (1/4 mile time). Below we practice a few simple functions to find out information about the dataset. 
...
In addition to `mpg` (miles per gallon), `qsec` (1/4 mile time), the other variable I am interested in is  `am` (transmission; 0 = automatic, 1 = manual).[^4]
